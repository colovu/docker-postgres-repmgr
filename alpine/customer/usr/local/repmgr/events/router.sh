...
LOG_I "Looking for the script: ${event_script}"
if [[ -f "${event_script}" ]]; then
    LOG_I "[REPMGR EVENT] will execute script '${event_script}' for the event"
    . "$event_script"
else
    LOG_I "[REPMGR EVENT] no script '${event_script}' found. Skipping..."
fi
